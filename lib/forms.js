...
var forms = {
  ...
  initConnection : function(options, cb) {
    var self = this;
    var uri = options.uri;
    var submissionUri = options.submissionUri;
    var key = options.key || uri;
    var submissionKey = options.submissionKey || submissionUri;
    var config = options.config || {};
    if ( self.connections[key] && !submissionUri) {
    ...
    if (submissionUri) {
      tasks.submissionMongooseConnection = async.apply(setupConnections.getMongooseConnection, submissionUri, forms.submissionSharedConnections.mongooseConnection);
      tasks.submissionMongoConnection = async.apply(setupConnections.getMongoConnection, submissionUri, forms.submissionSharedConnections.mongooseConnection);
    }

    async.series(tasks, function(err, requiredConnections) {
      if (err) {
      ...
      var submissionConnections;
      if (submissionKey) {
        submissionConnections = {
          "submissionMongooseConnection": requiredConnections.submissionMongooseConnection,
          "submissionDatabaseConnection": requiredConnections.submissionMongoConnection
        };
        self.connections[submissionKey] = submissionConnections;
      }

      var filter;
      if (!self.connections[submissionKey]) {
        models.init(connections.mongooseConnection, config);
      } else {
        filter = _.filter(Object.keys(MODELNAMES), function(k) {
          return k !== 'FORM_SUBMISSION';
        });
        models.init(self.connections[key].mongooseConnection, config, filter);

        filter = ['FORM_SUBMISSION'];
        models.init(self.connections[submissionKey].mongooseConnection, config, filter);
      }

      return cb(undefined, connections, submissionConnections);
    });
  },
   ...
   */
  setSharedConnections: function(dbConnections, isSubmissionConnection) {
    var key;
    if (isSubmissionConnection) {
      key = 'submissionSharedConnections';
    } else {
