...
var getForm = function(connections, options, cb) {
  async.waterfall([validateParams, sanitizeFormName, getFormUsingId, populateFieldDataFromDataSources, constructResultJSON], function(err, resultJSON) {
    if (err) {
  ...
  function sanitizeFormName() {
    options.name = ESAPI.encoder().encodeForHTML(options.name);
  }
